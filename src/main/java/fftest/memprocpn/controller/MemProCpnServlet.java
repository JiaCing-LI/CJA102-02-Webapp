...
import fftest.memprocpn.service.MemProCpnServiceImpl;
...
public class MemProCpnServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req.getRequestDispatcher(
						"/backend/memprocpn/select_page.jsp");
			...
			MemProCpnVO memProCpnVO = memProCpnSvc
					.getOneMemProCpn(cpnHolderDetailId);
			if (memProCpnVO == null) {
				errorMsgs.add("查無資料");
			}
			// Send the use back to the form, if there were errors
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllMemProCpn.jsp的請求
			 ...
			 * 1.接收請求參數
			 ****************************************/
			Integer cpnHolderDetailId = Integer
					.valueOf(req.getParameter("cpnHolderDetailId"));
		...
		if ("updateStatus".equals(action)) { // 來自update_MemProCpn_input.jsp的請求
			...
			Integer cpnHolderDetailId = Integer
					.valueOf(req.getParameter("cpnHolderDetailId"));
			Byte cpnUseStatus = (req.getParameter("cpnUseStatus") != null)
					? (byte) 1 : (byte) 0;
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher(
						"/backend/memprocpn/update_memprocpn_input.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
			...
			memProCpnVO = memProCpnSvc.updateStatusMemProCpn(cpnHolderDetailId,
					cpnUseStatus);
		...
		if ("insert".equals(action)) { // 來自 addMemProCpn.jsp 的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher(
							"/backend/memprocpn/addMemProCpn.jsp");
					failureView.forward(req, res);
					return;
				}
